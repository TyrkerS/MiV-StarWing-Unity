...
{
    [SerializeField]
    private Transform target; // Objetivo al que las balas rodearán
    [SerializeField]
    private GameObject bulletPrefab; // Prefab de la bala a instanciar
    [SerializeField]
    private Transform firePointLeft; // Punto base izquierdo desde donde se instanciarán las balas
    [SerializeField]
    private Transform firePointRight; // Punto base derecho desde donde se instanciarán las balas
    [SerializeField]
    ...
    [SerializeField]
    private float fireRadius = 7f; // Radio alrededor del objetivo para disparar las balas
    ...
    private void ShootAroundTarget()
    {
        if (Time.time >= nextShotTime && target != null)
        {
            // Calcular una posición aleatoria alrededor del target dentro del radio
            Vector3 randomOffset = Random.insideUnitSphere * fireRadius;
            Vector3 spawnPosition = target.position + randomOffset;

            // Calcular la dirección hacia el objetivo
            Vector3 direction = (spawnPosition - firePointLeft.position).normalized;

            // Instanciar la bala
            GameObject bullet = Instantiate(bulletPrefab, firePointLeft.position, Quaternion.identity);
            ...
            // Calcular una posición aleatoria alrededor del target dentro del radio
            Vector3 randomOffset2 = Random.insideUnitSphere * fireRadius;
            Vector3 spawnPosition2 = target.position + randomOffset2;

            // Calcular la dirección hacia el objetivo
            Vector3 direction2 = (spawnPosition2 - firePointRight.position).normalized;

            // Instanciar la bala
            GameObject bullet2 = Instantiate(bulletPrefab, firePointRight.position, Quaternion.identity);

            // Configurar la dirección y el daño de la bala
            ScriptShootEnemy bulletScript2 = bullet2.GetComponent<ScriptShootEnemy>();
            if (bulletScript != null)
            {
                bulletScript2.SetDamage(damage);
                bulletScript2.SetDirection(direction);
            }
    ...
    private IEnumerator MoveToRandomPosition()
    {
        ...
        Debug.Log("Enemy2 moved to: " + transform.position);
    }
}